...
There is a change that the selectors used to get the dom elements are obsolete. 
...
- `filters` can contain 3 keys: 
  1 - contractType
  2 - remote
  3 - experienceLevel

- `contractType`: The contract type filter. Can be a number or array of number. Available options:
...
- `remote`: The remote work option filter. Available options:
